...
import { roleRoutes, roleLabels, roleModuleAccess } from "@/lib/roles";
...
export function SettingsPageContent() {
  ...
  const handleSwitch = (role: string) => {
    switchModule(role);
    // Navigate to the role's home dashboard after switching
    const homeRoute = roleRoutes[role] ?? "/student-affairs/college-data/courses";
    ...
    <div className="min-h-full p-6 md:p-10 space-y-8">
      ...
      <div className="flex items-center gap-5">
        ...
        <div>
          ...
          {/* Active module badge */}
          {/* {user?.activeModule && (
            <span className="inline-block mt-2 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider bg-blue-600 text-white">
              {roleLabels[user.activeModule] ?? user.activeModule}
            </span>
          )} */}
        </div>
      </div>
