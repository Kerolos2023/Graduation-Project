...
export default function AcademicLevelsPage() {
  ...
  const [pageNumber, setPageNumber] = useState(1);
  const [pageSize] = useState(10);
  const [loading, setLoading] = useState(false); // خليناها false كبداية عشان ميعلقش
  const [submitting, setSubmitting] = useState(false);
  const [searchValue, setSearchValue] = useState("");
  ...
  const loadLevels = useCallback(async () => {
    ...
    try {
      ...
      const response = await levelService.getAllLevels(selectedProgramId, {
        PageNumber: pageNumber,
        PageSize: pageSize,
        // لو الـ API بتاعك بيدعم الـ Search سيرفر، هتباصي الـ searchValue هنا
      });
  ...
  // استدعاء الداتا عند جاهزية السياق الأكاديمي
  useEffect(() => {
  ...
  // تصغير الصفحة لـ 1 عند البحث (Client-side حالياً)
  ...
  // حمايات الـ UI في حال عدم اختيار برنامج
  if (!selectedProgramId) {
    return (
      <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-10 bg-[#F5F5F5] min-h-screen font-sans">
        <CollegeDataTabs />
