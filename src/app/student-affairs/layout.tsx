...
export default function StudentAffairsLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="flex h-[100dvh] w-full bg-[#e7e9ef] p-0 md:p-5 gap-0 md:gap-6 font-inter overflow-hidden relative">
            ...
            <div className={`fixed md:relative inset-y-0 left-0 z-50 transform transition-transform duration-300 ease-in-out md:translate-x-0 bg-transparent ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:block h-full shadow-2xl md:shadow-none p-4 md:p-0`}>
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden gap-4 md:gap-6 w-full p-4 md:p-0 bg-[#e7e9ef]">
                ...
                {/* Top Navigation Block with Mobile Menu Button */}
