...
export default function AddingExaminationCommitteeDistributionPage() {
  ...
  const termId = searchParams.get("termId") || searchParams.get("examTermId") || searchParams.get("examTermID") || "";
  const searchParamExamId = searchParams.get("examId") || searchParams.get("courseOfferingExamId") || "";
  ...
  useEffect(() => {
    const initPageData = async () => {
      if (!termId) {
        console.warn("termId is missing from search parameters.");
        setIsLoading(false);
        return;
      }
      try {
        ...
        if (isUpdateState) {
          console.log("Fetching existing exam details and committees for id:", searchParamExamId);

          const [examDetails, assignedCommitteesData] = await Promise.all([
            committeeDistributionService.getExamDetails(termId, searchParamExamId),
            committeeDistributionService.getAssignedExamCommittees(termId, searchParamExamId)
          ]);
          ...
          if (assignedCommitteesData && Array.isArray(assignedCommitteesData)) {
            const mappedCommittees = assignedCommitteesData.map((c: any) => ({
              id: c.id,
              name: c.committeeName || `Committee ${c.committeeNumber || c.id.substring(0, 4)}`,
  ...
  const handleToggleCommitteeSelect = (id: string) => {
    if (selectedCommitteeIds.includes(id)) {
      setSelectedCommitteeIds(selectedCommitteeIds.filter((item) => item !== id));
    } else {
      setSelectedCommitteeIds([...selectedCommitteeIds, id]);
    }
  };

  const handleAddSelectedCommittees = () => {
    if (selectedCommitteeIds.length === 0) return;

    const newCommitteesToAppend: any[] = [];
    selectedCommitteeIds.forEach((id) => {
      const committee = availableCommittees.find((c) => c.id === id);
      if (committee && !addedCommittees.some((c) => c.id === id)) {
        newCommitteesToAppend.push({
          id: committee.id,
          name: committee.committeeName || `Committee ${committee.committeeNumber}`,
    ...
    setAddedCommittees([...addedCommittees, ...newCommitteesToAppend]);
  ...
  const handleGlobalSave = async () => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.errors?.request?.[0] || error.response?.data?.detail || error.response?.data?.message || "Failed to save data.";
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pages.push(
        <button
          ...
          className={`w-9 h-9 flex items-center justify-center rounded-xl transition-colors font-bold text-xs cursor-pointer ${isCurrent
    ...
    <div className="w-full h-full flex flex-col gap-6 font-inter pb-8" ref={topRef} dir="ltr">
      ...
      <div className="bg-white rounded-[24px] p-6 shadow-[0_4px_24px_rgba(0,0,0,0.02)] border border-[#eaebf0] shrink-0 scroll-mt-6">
        ...
        <div className="space-y-6">
          ...
          <div className="flex flex-col gap-1.5 w-full">
            <label className="text-[13px] font-bold text-gray-900 ml-1">Committees (Multi-Select)</label>
            <Select
              onOpenChange={(open) => {
                if (!open) {
            ...
            >
              <SelectTrigger className="w-full px-4 py-2.5 rounded-[12px] border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm shadow-sm font-medium h-auto text-gray-700">
                <SelectValue placeholder={selectedCommitteeIds.length > 0 ? `${selectedCommitteeIds.length} Committees Selected` : "Select Committees"} />
              </SelectTrigger>

              <SelectContent
                className="rounded-xl max-h-60 overflow-y-auto bg-white border border-gray-100 shadow-md p-1 space-y-0.5"
                onCloseAutoFocus={(e) => e.preventDefault()}
              >
                {availableCommittees.map((committee) => {
                  const isChecked = selectedCommitteeIds.includes(committee.id);
                  const isAlreadyAdded = addedCommittees.some((added) => added.id === committee.id);
                  ...
                  return (
                    <div
                      ...
                      onClick={(e) => {
                        ...
                        handleToggleCommitteeSelect(committee.id);
                      }}
                      className={`flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer text-sm font-medium transition-colors ${isAlreadyAdded ? 'opacity-40 cursor-not-allowed bg-gray-50/50' : 'hover:bg-gray-50 text-gray-700'
                        }`}
                    >
                      <input
                        type="checkbox"
                        ...
                        readOnly
                        className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-600 cursor-pointer disabled:opacity-50"
                      />
                      <span className={isAlreadyAdded ? "line-through text-gray-400" : ""}>
                        {committee.committeeName || `Committee ${committee.committeeNumber}`} — {committee.place} (Capacity: {committee.maxCapacity})
          ...
          <button
            type="button"
            onClick={handleAddSelectedCommittees}
            disabled={selectedCommitteeIds.length === 0}
            className="w-full text-white font-semibold py-3.5 rounded-[12px] transition-all shadow-sm cursor-pointer flex items-center justify-center text-sm active:scale-[0.99] bg-blue-600 hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed"
          >
            Add Selected Committees ({selectedCommitteeIds.length})
          </button>
        </div>
      </div>
          ...
          <div className="flex justify-center mt-6 pt-4 border-t border-gray-50">
            <div className="flex items-center gap-1.5 text-sm font-medium text-gray-600">
              <button
                ...
                className={`p-2 border border-gray-200 rounded-xl hover:bg-gray-50 text-gray-400 transition-colors flex items-center gap-1 ${currentPage === 1 ? "opacity-40 cursor-not-allowed" : "cursor-pointer"
                  }`}
              >
              ...
              <button
                ...
                className={`p-2 border rounded-xl hover:bg-gray-50 text-[#2563EB] border-blue-100 bg-white transition-colors flex items-center gap-1 ml-1 font-semibold ${currentPage === totalPages ? "opacity-40 cursor-not-allowed" : "cursor-pointer"
                  }`}
              >
                Next
