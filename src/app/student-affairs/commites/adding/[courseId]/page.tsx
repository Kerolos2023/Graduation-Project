...
export default function AddingExaminationCommitteeDistributionPage() {
  ...
  // 🎯 ديناميكي بالكامل: يقرأ الـ termId المبعوث للفاينال أو البراكتيكال أوتوماتيك
  const termId = searchParams.get("termId") || ""; 
  const searchParamExamId = searchParams.get("examId") || "";
  const courseName = searchParams.get("courseName") || "Course Subject";

  // فحص ما إذا كانت مادة خضراء (تعديل) أم مادة جديدة (إضافة)
  const isUpdateState = (
    searchParamExamId && 
  ...
  // جلب اللجان وتفاصيل المادة مسبقاً
  useEffect(() => {
    const initPageData = async () => {
      ...
      try {
        ...
        // 2. لو مادة خضراء، اسحب تفاصيل الامتحان
        if (isUpdateState) {
  ...
  const handleGlobalSave = async () => {
    ...
    try {
      if (isUpdateState) {
        // 🎯 الـ Payload المعدل الموحد بالملّي طبقاً لتعديل الباك إند الأخير (مصفوفة Strings)
        const putPayload = {
          date: formData.day,
          startTime: cleanStartTime,
          endTime: cleanEndTime,
          ExamCommitteesIds: addedCommittees.map((c) => c.id) // 🎯 مصفوفة سترنجات عادية ["guid"] منعاً للـ 400
        };

        console.log("Executing direct PUT route matching updated backend...");
        ...
        const postPayload = {
          ...
          startTime: cleanStartTime,
          endTime: cleanEndTime,
          ExamCommitteesIds: addedCommittees.map((c) => c.id) // 🎯 مصفوفة سترنجات عادية
        };
  ...
  return (
    <div className="w-full min-h-screen bg-[#F5F5F5] p-4 md:p-8 flex flex-col gap-6 font-sans antialiased" ref={topRef} dir="ltr">
      ...
      {statusMessage.type && (
        <div className={`border px-5 py-4 rounded-xl text-sm transition-all shadow-sm max-w-6xl w-full mx-auto ${
